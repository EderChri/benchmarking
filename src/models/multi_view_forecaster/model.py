...
class ForecastHead(nn.Module):
    def __init__(self, args):
        ...
        if args.feature == "latent":
            ...
            fc_in_dim = len(args.loss_type) * args.num_hidden
            ...
            raise ValueError(f"Unknown feature mode '{args.feature}'. Use 'latent' or 'hidden'.")

        self.fc = nn.Linear(fc_in_dim, args.forecast_horizon * self.num_out)
    ...
    def forward(self, xt, dx, xf):
        ...
        if self.args.feature == "latent":
            zt, zd, zf_v = xt, dx, xf
            if self.args.loss_type == "ALL":
                stacked = torch.stack([zt, zd, zf_v], dim=1)  # [batch, 3, hidden]
                attended = self.self_attention(stacked)[0] + stacked
                zt, zd, zf_v = attended[:, 0], attended[:, 1], attended[:, 2]
            emb = self._select_domains(zt, zd, zf_v)
            ...
            zf_v = self.output_layer_f(torch.cat([hf.mean(dim=1), hf_i.mean(dim=1)], dim=-1))
            emb = self._select_domains(zt, zd, zf_v)

        out = self.fc(emb).reshape(emb.shape[0], self.args.forecast_horizon, self.num_out)
